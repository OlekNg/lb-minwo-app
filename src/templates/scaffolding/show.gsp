...
<html>
	...
	<body>
		<div id="show-${domainClass.propertyName}" class="content scaffold-show" role="main">
			...
			<div class="property-list form-horizontal ${domainClass.propertyName}">
			<%  excludedProps = Event.allEvents.toList() << 'id' << 'version'
				...
				<div class="form-group">
					<label id="${p.name}-label" class="property-label control-label col-sm-3"><g:message code="${domainClass.propertyName}.${p.name}.label" default="${p.naturalName}" /></label>
					<div class="col-sm-9">
					<%  if (p.isEnum()) { %>
					...
					<%  } else if (p.oneToMany || p.manyToMany) { %>
						<div class="list-group">
							...
							<g:link controller="${p.referencedDomainClass?.propertyName}" class="list-group-item" action="show" id="\${${p.name[0]}.id}">\${${p.name[0]}?.encodeAsHTML()}</g:link>
							</g:each>
						</div>
					<%  } else if (p.manyToOne || p.oneToOne) { %>
						<p class="property-value form-control-static" aria-labelledby="${p.name}-label"><g:link controller="${p.referencedDomainClass?.propertyName}" action="show" id="\${${propertyName}?.${p.name}?.id}">\${${propertyName}?.${p.name}?.encodeAsHTML()}</g:link></p>
					...
					<%  } else if (!p.type.isArray()) { %>
						<p class="property-value form-control-static" aria-labelledby="${p.name}-label"><g:fieldValue bean="\${${propertyName}}" field="${p.name}"/></p>
					<%  } %>
					</div>
				</div>
				</g:if>
